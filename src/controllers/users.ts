...
import Sandbox, { ISandbox } from '../models/Sandbox';
...
const deleteUser = asyncWrapper(async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    // @ts-ignore
    const userObjectId = req.params.id.toObjectId()
    if (!user) {
        return next(createCustomError('could not find the user id', 404))
    }

    const sandbox: ISandbox = await Sandbox.findOneAndDelete({ PlayerId: user._id || userObjectId }).exec()

    console.log({
        PlayerId: user._id || userObjectId,
        sandbox
    })

    res.status(200).json({ success: true, user, sandbox })
})
